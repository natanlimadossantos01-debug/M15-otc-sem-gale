# M15-otc-sem-gal#!/usr/bin/env python3
"""
⚛️ QUANTUM IA M15 - OTC - SEM GALE
🎯 Confiança mínima: 75%
📊 4 Pares OTC Confirmados
🛡️ Anti-pavio + Volatilidade
⏱️ Timeframe: M15 (15 min)
...
ANTECEDENCIA = 30
CONFIANCA_MINIMA = 75        # Muito alta (sem gale)
...
def banner():
    print("⚛️ QUANTUM IA M15 - OTC | SEM GALE | 4 Pares")
        ...
        for nome, ativo_id in ATIVOS_OTC.items():
            try:
                if not api.check_connect():
                    ...
                    if not api:
                        break
                c = api.get_candles(ativo_id, TIMEFRAME, 80, time.time())  # M15
                if c and len(c) > 0:
...
🏳ATIVO: {ativo}-OTC {'🟢' if direcao == 'CALL' else '🔴'}
📊 DIREÇÃO: {direcao}
🎯 CONFIANÇA: {conf:.0f}%
...
🎯 Assertividade: {tx}%"""
    ...
    async def executar(self):
        banner()
        print("⚛️ Bot M15 OTC sem gale iniciando...")
        self.tg.send(f"🔥 *QUANTUM IA M15 OTC - SEM GALE*\n📊 4 Pares\n🎯 Confiança {CONFIANCA_MINIMA}%+\n🚫 SEM GALE\n⏱️ Timeframe: 15 min\n🛡️ Anti-pavio")
